...
from kubernetes.client import V1ObjectMeta, AppsV1beta1Deployment, AppsV1beta1Api, AppsV1beta1DeploymentSpec, \
    V1PodTemplateSpec, V1PodSpec, V1Container, V1ContainerPort, V1EnvVar, V1DeleteOptions, V1Pod
...
class KubernetesDeploymentService:
    ...
    def get_pods_by_name(self, clients, namespace, app_name):
        """
        :param KubernetesClients clients:
        :param str namespace:
        :param str app_name:
        ...
        """
        query_selector = self._prepare_deployment_default_label_selector(app_name)
        items = clients.core_api.list_namespaced_pod(namespace=namespace, label_selector=query_selector).items
        if not items:
            return None
        return items
