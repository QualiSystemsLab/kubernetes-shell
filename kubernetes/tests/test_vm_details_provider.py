...
class TestVmDetailsProvider(unittest.TestCase):
    ...
    def test_create_vm_details_from_deployment_and_services_with_type_load_balancer(self):
        # arrange
        deployment = MagicMock()
        deployment.spec.template.spec.containers = [Mock(image='ubuntu:16.04'), Mock(image='ubuntu:16.04'), Mock(image='ubuntu:14.04')]
    ...
    def test_create_vm_details_from_deployment_and_services_with_type_load_nodeport(self):
        # arrange
        deployment = MagicMock()
        deployment.spec.template.spec.containers = [Mock(image='ubuntu:16.04'), Mock(image='ubuntu:16.04'),
                                                    Mock(image='ubuntu:14.04')]
        ...
        port1 = Mock()
        port2 = Mock()
        external_service.spec.ports = [port1, port2]
        external_service.spec.type = 'NodePort'
        ...
        self.assertEquals(self._get_vm_prop(result, 'External Ports').value,
                          "{0}:{1}, {2}:{3}"
                          .format(port1.port, port1.node_port, port2.port, port2.node_port))
